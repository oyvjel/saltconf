...
# Windows check
    if os.path.exists(os.path.join(mpt, 'salt/conf/pki')):
        tmp = os.path.join(mpt, 'salt/conf')
        # Write the new minion's config to a tmp file
        tmp_config = os.path.join(tmp, 'minion_id')
        with salt.utils.fopen(tmp_config, 'w+') as fp_:
            fp_.write(id_)
#            fp_.write(yaml.dump(config, default_flow_style=False))
            
        # Generate keys for the minion
        tmp_pki = os.path.join(tmp, 'pki/minion')
        salt.crypt.gen_keys(tmp_pki, 'minion', 2048)
        pubkeyfn = os.path.join(tmp_pki, 'minion.pub')
#        privkeyfn = os.path.join(tmp_pki, 'minion.pem')
...
#        if approve_key:
        ...
        _umount(mpt, ftype)    
        return 'Preinstalled Windows guest {0} configured and seeded'.format(id_)

#    else:
    INSTALL_DIR = os.path.dirname(os.path.realpath(salt.syspaths.THIS_FILE))
...
def _install(mpt):
    ...
    shutil.copy(BOOTSTRAP, os.path.join(mpt, 'tmp'))
    
    _check_resolv(mpt)
    # Exec the chroot command
    cmd = 'if type salt-minion; then exit 0; '
    cmd += 'else sh /tmp/bootstrap-salt.sh -c /tmp; fi'
#   cmd += 'else sh {0} -c /tmp; fi'.format(BOOTSTRAP)
    return not _chroot_exec(mpt, cmd)

def _install_old(mpt):
    '''
    Determine whether salt-minion is installed and, if not,
    install it.
    Return True if install is successful or already installed.
...
def _check_install(root):
    '''
    # øj:can not run linux command on windows chroot; test file existance
    #    if os.path.isfile(os.path.join(root, 'salt/conf/pki')):
        må generer minion.pem  minion.pub  minion_master.pub
    '''    
